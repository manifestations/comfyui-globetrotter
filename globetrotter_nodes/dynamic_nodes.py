...
def load_attire_options(country_code):
    """
    Loads attire options for a specific country by directly checking the `body_part` entry in the JSON file.
    """
                ...
                with open(json_path, 'r', encoding='utf-8') as f:
                    try:
                        ...
                        if body_part and "attires" in data:
                            options = ["none", "random"] + [attire["name"] for attire in data["attires"]]
                            # Capitalize each word in each option except 'none' and 'random'
                            options = [opt if opt in ["none", "random"] else opt.title() for opt in options]
...
def load_appearance_options(country_code: str):
    """Load region appearance options for a country from data/appearance/<country>.json."""
    appearance_file = os.path.join(BASE_DIR, '..', 'data', 'appearance', f'{country_code}.json')
...
def create_attire_node(country: dict) -> tuple[str, type]:
    ...
    def input_types(cls):
        ...
        required = {
            ...
            "age": (["unspecified", "infant", "toddler", "young kid", "older kid", "teenager", "young adult", "adult", "middle aged", "elderly"],),
        }
        ...
        required["random_mode"] = (["random", "fixed", "increment", "decrement"], {"default": "random"})
        required["seed"] = ("INT", {"default": 0, "min": 0, "max": 2**32-1})
        required["lora_trigger"] = ("STRING", {"multiline": False, "default": ""})
        required["custom_prompt"] = ("STRING", {"multiline": True, "default": ""})
        return {"required": required}

    def generate_prompt(self, region: str, hair_style: str, gender: str, age: str, random_mode: str, seed: int, lora_trigger: str, custom_prompt: str, **kwargs) -> tuple[str]:
        ...
        region_info = next((r for r in region_options if r['name'] == region), None)
        region_display = region if region and region != "Unspecified" and region_info else None

        # Add LORA trigger phrase or fallback
        if lora_trigger.strip():
            if region_display:
                prompt_parts.append(f"A {age} {gender} called {lora_trigger.strip()} from {region_display}, {country_name}")
            else:
                prompt_parts.append(f"A {age} {gender} called {lora_trigger.strip()} from {country_name}")
        else:
            if region_display:
                prompt_parts.append(f"A {age} {gender} from {region_display}, {country_name}")
            else:
