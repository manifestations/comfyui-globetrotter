...
class OllamaLLMNode:
    ...
    def INPUT_TYPES(cls):
        ...
        try:
            ...
            cls.art_styles = cls.load_from_json(os.path.join(styles_dir, 'art_styles.json'))
            ...
            if os.path.exists(prompts_dir):
                for f in os.listdir(prompts_dir):
                    if f.endswith('.json'):
                        ...
                        content = cls.load_from_json(os.path.join(prompts_dir, f))
                        if 'instructions' in content:
                            cls.prompt_instructions[style_name] = content['instructions']
            
            installed_models = cls.get_ollama_models("http://127.0.0.1:11434/api/generate")

            return {
                "required": {
                    "keywords": ("STRING", {"multiline": True, "default": "a cat, sitting on a chair, fantasy art"}),
                    "custom_prompt": ("STRING", {"multiline": True, "default": ""}),
                    "model_name": (["disabled"] + installed_models, {"default": "disabled"}),
                    "prompt_style": (["random", "disabled"] + list(cls.prompt_instructions.keys()), {"default": "random"}),
                    "art_style": (["random", "disabled"] + list(cls.art_styles.keys()), {"default": "random"}),
            ...
            return {
                "required": {
                    "keywords": ("STRING", {"multiline": True, "default": f"ERROR: Node failed to load. Check console for details: {e}"}),
                }
            }
    ...
    def generate_prompt(self, keywords, custom_prompt="", model_name="disabled", prompt_style="random", art_style="random", seed=0, ollama_url="http://127.0.0.1:11434/api/generate"):
        ...
        if prompt_style == "random":
            prompt_style = random.choice(list(self.prompt_instructions.keys())) if self.prompt_instructions else "disabled"
        if art_style == "random":
            art_style = random.choice(list(self.art_styles.keys())) if self.art_styles else "disabled"
        ...
        instructions = self.prompt_instructions.get(prompt_style, "Generate a detailed prompt:")
        ...
        try:
            ...
            error_message = f"OllamaLLMNode Error: Could not connect to Ollama. Details: {e}"
            print(error_message)
            return (f"ERROR: OLLAMA NOT REACHABLE. Keywords: {keywords}",)
        except Exception as e:
            error_message = f"OllamaLLMNode Error: An unexpected error occurred: {e}"
            print(error_message)
